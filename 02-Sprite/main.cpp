...
vector<string> ReadFile()
{
	...
	outputFile.open("ImageAndImageLocation.txt", ios::in);
...
void LoadResources()
{
	...
	wstring output(data[0].begin(), data[0].end());
	textures->Add(ID_TEX_MARIO, output.c_str(), D3DCOLOR_XRGB(176, 224, 248));

	wstring output1(data[1].begin(), data[1].end());
	textures->Add(ID_TEX_ENEMY, output1.c_str(), D3DCOLOR_XRGB(156, 219, 239));
	...
	// Phần của con rùa
	LPDIRECT3DTEXTURE9 turtleTex = textures->Get(ID_TEX_ENEMY);
	LPDIRECT3DTEXTURE9 texMario = textures->Get(ID_TEX_MARIO);

	for (unsigned i = 2; i < data.size(); i++)
	{
		istringstream iss(data[i]);
		vector<string> location{ istream_iterator<string>{iss},
								 istream_iterator<string>{} };
		if (i < 8)
			sprites->Add(stoi(location[0]), stoi(location[1]), stoi(location[2]), stoi(location[3]), stoi(location[4]), texMario);
		else
			sprites->Add(stoi(location[0]), stoi(location[1]), stoi(location[2]), stoi(location[3]), stoi(location[4]), turtleTex);
	}
	...
	*/
	...
	turtle->AddAnimation(600);
	turtle->AddAnimation(601);

	turtle->SetPosition(20.0f, 150.0f);
}
