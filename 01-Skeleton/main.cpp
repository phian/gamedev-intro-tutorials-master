...
void LoadResources()
{
	for (int i = 0; i < 9; i++)
	{
		if (i < 3)
		{
			object[i] = new CMario(MARIO_TEXTURE_PATH);
			object[i]->SetPosition(10.0f, 130.0f + 16 * i);
		}

		else if (i < 6)
		{
			object[i] = new CBall(BALL_TEXTURE_PATH);
			object[i]->SetPosition(10.0f + 32 * i, 20.0f + 32 * i);
		}	

		else
		{
			object[i] = new CGameObject(BRICK_TEXTURE_PATH);
			object[i]->SetPosition(10.0f + 16 * i, 100.0f);
		}
	}
}
